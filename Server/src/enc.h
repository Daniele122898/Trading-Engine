...
unsigned int sha256(unsigned char *text, size_t text_len, unsigned char *hashed) {
    EVP_MD_CTX* context;
...
unsigned int sha256_salted(std::string & input, unsigned char *hashed, unsigned char *salt) {
    get_rand(salt, 32);
...
bool sha256_match(std::basic_string<std::byte>& pwhash, std::basic_string<std::byte>& salt, std::string& password) {
