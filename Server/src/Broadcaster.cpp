...
namespace TradingEngine {
    ...
    inline bool IsLoggedIn(crow::websocket::connection & conn, std::unordered_map<uint64_t, crow::websocket::connection*>& users) {
        auto it = users.find((uint64_t)conn.userdata());
        if (it == users.end()) {
            RespondError(conn, WsData::ErrorCodes::UNAUTHORIZED, "You have to log into the websocket before anything else!");
            return false;
        }
        return true;
    }
    ...
    void
    Broadcaster::ReportOrderFill(const Data::Order &order, const Data::Order &counterOrder, Data::FillReason reason,
                                 uint32_t diff) {
        auto sid = order.SymbolId;
        auto wsconns = m_symbolsToUsers.find(sid);
        for (auto conn: wsconns->second) {
            conn->send_text("You subscribed to this shit");
        }
    }
        ...
        try {
            ...
            switch (request.opcode) {
                case WsData::OpCodes::LOGIN: {
                    ...
                    if (it == m_apiKeys.end()) {
                        RespondError(conn, WsData::ErrorCodes::UNAUTHORIZED, "ApiKey not found, please login Rest route before using the websocket");
                        return;
                    }
                ...
                case WsData::OpCodes::SUBSCRIBE: {
                    ...
                    for (auto sid : sub.symbolIds) {
                        ...
                        RespondError(conn, WsData::ErrorCodes::NOT_FOUND, "Symbol with Id " + std::to_string(sid) + " could not be found");
                        return;
                    }
            ...
            Respond(conn, WsData::OpCodes::ERROR, WsData::Error{WsData::ErrorCodes::INVALID_JSON, "Provide proper JSON formatted data"});
